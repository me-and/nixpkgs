...
stdenv.mkDerivation (
  ...
  // {
    ...
    postInstall =
      ...
      in
      assert lib.versionAtLeast version "10.1";
      ...
      + lib.optionalString supportFlags.gstreamerSupport ''
        ...
        # by setting WINELOADER to point to the original binary. The locations
        ...
        # in dlls/kernel32/process.c).  This works with the current implementation of
        # wrapProgram, but is more fragile than one might like.
        for i in wine wine64 ; do
          prog="$out/bin/$i"
          if [ -e "$prog" ]; then
            wrapProgram "$prog" \
                --set WINELOADER "$out/libexec/$i" \
